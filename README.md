...
flowchart TD
    ...
    U --> V[fa:fa-map bert vectors]
    O --> R(fa:fa-bar-chart visualize_results.py)
    P --> R
    Q --> R
    Q --> S(fa:fa-bar-chart visualize_w2v_cluster.py)
    Q --> W(fa:fa-bar-chart visualize_w2v_3d.py)
    V --> X(fa:fa-bar-chart visualize_bert_3d.py)
```
...
- [x] Criar um script vectorize_bert.py, utilizando o BERTugues para vetorização dos produtos (verificar se é preciso gerar um corpus específico para este caso)
