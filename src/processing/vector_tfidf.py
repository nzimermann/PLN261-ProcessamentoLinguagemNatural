"""
Etapa 2 do pipeline de clusterização — Vetorização TF-IDF.

Lê data/processed/tokens_products_filtered.jsonl e transforma os lemmas
de cada produto em vetores numéricos usando TF-IDF com bigrams.
...
Saída:
    data/processed/tfidf_matrix.npz     — matriz esparsa TF-IDF
    data/processed/tfidf_skus.json      — lista de SKUs na ordem das linhas
    data/processed/tfidf_features.json  — lista de features (vocab) na ordem das colunas
...
OUTPUT_MATRIX = PROCESSED_DIR / "tfidf_matrix.npz"
OUTPUT_SKUS = PROCESSED_DIR / "tfidf_skus.json"
OUTPUT_FEATURES = PROCESSED_DIR / "tfidf_features.json"

# TF-IDF — parâmetros como constantes tipadas individualmente
NGRAM_RANGE: tuple[int, int] = (1, 1)  # unigrams e bigrams
...
NORM: str = "l2"  # normalização por comprimento do vetor
...
# o formato que o TfidfVectorizer espera como entrada.
# ============================================================================
...
with open(FILTERED_JSONL, encoding="utf-8") as f:
    for linha in f:
        ...
        # Junta lemmas em uma string — formato esperado pelo TfidfVectorizer
        documentos.append(
...
print(f"Amostra de bigrams          : {bigrams[:20]}")


# ============================================================================
# SEÇÃO 3 — TRANSFORMAÇÃO
# Aplica o vocabulário aprendido a cada documento e produz a matriz TF-IDF.
# A matriz é esparsa: a maioria dos valores é zero (cada produto usa uma
# fração pequena do vocabulário total).
# ============================================================================
...
# Para cada produto, mostra quais features têm maior peso TF-IDF.
# Esses são os termos que mais caracterizam aquele produto em relação
# ao conjunto inteiro — exatamente o que a clusterização vai usar.
# ============================================================================
...
for idx in indices_amostra:
    ...
    top = [(vocabulario[i], round(float(vetor[i]), 4)) for i in top_idx if vetor[i] > 0]
    print(f"\nSKU {skus[idx]}:")
    for feature, peso in top:
        print(f"  {feature:<30} {peso}")


# ============================================================================
# SEÇÃO 5 — DIAGNÓSTICO DE FEATURES GLOBAIS
# Mostra quais features têm maior peso agregado no corpus inteiro.
# Features com peso muito alto em todo o corpus são candidatas a ajuste
# no max_df — podem estar poluindo a separação entre clusters.
# ============================================================================
...
top_global_idx = pesos_globais.argsort()[::-1][:20]

print("Top 20 features por peso acumulado no corpus:")
for i in top_global_idx:
    print(f"  {vocabulario[i]:<35} {pesos_globais[i]:.2f}")
...
print("\nVetorização concluída. Próxima etapa: clusterização.")
